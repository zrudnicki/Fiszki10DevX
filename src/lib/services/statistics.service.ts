...
export class StatisticsService {
  ...
  async getGenerationStats(): Promise<GenerationStatsDTO> {
    try {
      ...
      const acceptance_rate = data.total_generated > 0 
        ? (totalAccepted / data.total_generated) * 100 
        : 0;
      const edit_rate = totalAccepted > 0 
        ? (data.total_accepted_edited / totalAccepted) * 100 
        : 0;
  ...
  async updateGenerationStats(updates: UpdateGenerationStatsRequest): Promise<GenerationStatsDTO> {
    try {
      ...
      const acceptance_rate = result.total_generated > 0 
        ? (totalAccepted / result.total_generated) * 100 
        : 0;
      const edit_rate = totalAccepted > 0 
        ? (result.total_accepted_edited / totalAccepted) * 100 
        : 0;
  ...
  async getLearningStats(query: LearningStatsQuery = {}): Promise<LearningStatsDTO> {
    try {
      ...
      return {
        ...
        accuracy_rate: mockReviewsByPeriod.length > 0 
  ...
  private generateMockReviewData(
    ...
    const reviews: Array<{ date: string; count: number; accuracy: number }> = [];
   ...
   */
  async incrementGenerationStats(
    generated: number = 1,
